...
var zen_coding = (function() {
		...
		function splitByLines(text, remove_empty) {
				var nl = getNewline(), lines = text.split(new RegExp(
								'\\r?\\n|\\n\\r|\\r|' + nl));
		 ...
		 *
		 * @param {String}
		 *			text Текст, который нужно отбить
		 * @param {String|Number}
		 *			pad Количество отступов или сам отступ
		 * @return {String}
		 */
		 ...
		 * используется для проверки принадлежности символа '>' аббревиатуре или
		 * тэгу
		 *
		 * @param {String}
		 *			str
		 ...
		 *			name Имя тэга
		 * @param {Number}
		 *			count Сколько раз вывести тэг (по умолчанию: 1)
		 * @param {String}
		 *			type Тип тэга (html, xml)
		 */
		...
		Tag.prototype = {
				 ...
				 */
				isInline : function() {
						return (this.name in getElementsCollection(this._res,
										'inline_level'));
				},
				...
				/**
				 * Проверяет, есть ли блочные потомки у текущего тэга. Используется для
				 * форматирования
				 *
				 * @return {Boolean}
				 */
				...
				toString : function(profile_name) {
						...
						var result = [], profile = (profile_name in profiles) ? profiles[profile_name]
										: profiles['plain'], attrs = '', content = '', start_tag = '', end_tag = '', cursor = profile.place_cursor ? '|'
										: '', self_closing = '', attr_quote = profile.attr_quotes == 'single' ? "'"
										: '"', attr_name;
						...
						function allowNewline(tag) {
								return (profile.tag_nl === true || (profile.tag_nl == 'decide' && tag
												.isBlock()))
						}
						...
						for ( var i = 0; i < this.attributes.length; i++) {
								...
								attr_name = (profile.attr_case == 'upper') ? a.name
												.toUpperCase() : a.name.toLowerCase();
						...
						if (this.name) {
								var tag_name = (profile.tag_case == 'upper') ? this.name
												.toUpperCase() : this.name.toLowerCase();
								if (this.isEmpty()) {
						...
						if (this.repeat_by_lines) {
								...
								for ( var j = 0; j < lines.length; j++) {
										...
										var elem_str = start_tag.replace(/\$/g, j + 1)
														+ cur_content + content + end_tag;
										result.push(elem_str.replace(content_placeholder,
														trim(lines[j])));
								}
						}
						...
						if (!result.length) {
								if (this.getContent()) {
										var pad = (profile.tag_nl === true || (this
														.hasTagsInContent() && this.isBlock())) ? 1 : 0;
		...
		Snippet.prototype = {
				...
				toString : function(profile_name) {
						var content = '', profile = (profile_name in profiles) ? profiles[profile_name]
										: profiles['plain'], result = [], data = this.value, begin = '', end = '', child_padding = '', child_token = '${child}';
						...
						for ( var i = 0; i < this.children.length; i++) {
								...
								if (i != this.children.length - 1
												&& (profile.tag_nl === true || (profile.tag_nl == 'decide' && this.children[i]
																.isBlock())))
										content += getNewline();
						}
		...
		return {
				...
				parseIntoTree : function(abbr, type) {
						...
						abbr = abbr
										.replace(
														re,
														function(str, operator, tag_name, id, class_name,
																		has_multiplier, multiplier, has_expando) {
																var multiply_by_lines = (has_multiplier && !multiplier);
																multiplier = multiplier ? parseInt(multiplier)
																				: 1;

																if (has_expando)
																		tag_name += '+';

																var current = isShippet(tag_name, type) ? new Snippet(
																				tag_name, multiplier, type)
																				: new Tag(tag_name, multiplier, type);
																if (id)
																		current.addAttribute('id', id.substr(1));

																if (class_name)
																		current.addAttribute('class', class_name
																						.substr(1).replace(/\./g, ' '));
						...
						function parseAbbreviations(obj) {
								for ( var key in obj) {
										...
										if (key.substr(-1) == '+') {
												...
												obj[key] = makeAbbreviation(key, m[1], m[2],
																m[4] == '/');
										} else {
		...
		var HTMLParser = this.HTMLParser = function(html, handler) {
				...
				while (html && !handler.stop) {
						...
						if (!stack.last() || !special[stack.last()]) {
								...
								var end_tag = '</' + stack.last() + '>', end_tag_ix = html
												.indexOf(end_tag);
...
(function($) {
		$.fn.zen = function(input) {
				...
				get = input.replace(/([.[*:+][a-zA-Z0-9]*)|(#.*\$[a-zA-Z0-9]*)/g, '')
								.replace(/>/g, ' ');
